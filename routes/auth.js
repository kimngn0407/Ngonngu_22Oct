...
router.post('/avatar', Authentication, Authorization("ADMIN", "MOD", "USER"), uploadAFileWithField('image'), async function (req, res, next) {
  try {
    if (!req.file) {
      Response(res, 400, false, "No image uploaded")
      return;
    }
    let user = await users.findById(req.userId);
    if (!user) {
